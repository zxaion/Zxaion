...
const utils = {
    ...
    // FIX S-8: Hapus dead code — return url apa adanya, normalisasi relative URL ke absolute
    getImagePreviewUrl(url, isDtreasure = false) {
        if (!url) return '';
        // Normalisasi relative URL ke absolute
        if (!url.startsWith('http')) return `${API_BASE}${url}`;
        return url;
        // Catatan: untuk DTREASURE, token ditambahkan oleh caller, bukan di sini
    },
...
const api = {
    ...
    // FIX S-17: Cache menyimpan data yang sudah difilter — bukan raw data
    // Sebelumnya cache.main menyimpan raw API response (termasuk HEADER images)
    // lalu saat cache hit langsung assign ke state.allPhotos tanpa difilter ulang
    async fetchImages() {
        try {
            const now = Date.now();
            if (this.imageCache.main && (now - this.imageCache.timestamp) < 300000) {
                // Cache sudah bersih (tersimpan setelah difilter), langsung pakai
    ...
    // FIX S-7: invalidateCache dipindahkan ke dalam object api dengan indentasi benar
    invalidateCache() {
    ...
    // FIX S-18: Kirim X-User-Token header agar worker tidak treat sebagai anonymous
    // Tanpa token, setelah fix W-12/W-13, statistik tidak akan pernah tercatat
...
const ui = {
    ...
    // FIX S-2:
    // - DTREASURE: tambahkan ?token= ke src img (browser img tag tidak bisa kirim header)
    // - DTREASURE: blokir klik-kanan dan drag pada gambar
    // - Download btn: jangan expose raw URL sebagai href untuk DTREASURE
    async openImageModal(photo) {
        ...
        if (isDtreasure) {
            // Tambahkan token sebagai query param — satu-satunya cara untuk browser img tag
            const token    = getOrCreateUserToken();
            const imageUrl = `${baseUrl}?token=${encodeURIComponent(token)}`;

            elements.modalImg.src = imageUrl;

            // Blokir klik-kanan & drag untuk proteksi DTREASURE
            elements.modalImg.oncontextmenu = (e) => e.preventDefault();
            elements.modalImg.setAttribute('draggable', 'false');

            // Download btn: jangan expose URL, alihkan ke handleDownload
            ...
            // Non-DTREASURE: hapus proteksi jika sebelumnya ada
            elements.modalImg.src             = baseUrl;
            elements.modalImg.oncontextmenu   = null;
            elements.modalImg.setAttribute('draggable', 'true');

            elements.downloadBtn.href    = `${baseUrl}?download=true`;
    ...
    // FIX S-3:
    // - Single shared IntersectionObserver (bukan satu per item — memory leak)
    // - Token ditambahkan ke data-src DTREASURE (bukan URL telanjang di DOM)
    // - img DTREASURE: draggable=false + oncontextmenu=return false
    // - Download button DTREASURE di search results: tampilkan lock + "10 Credits"
    renderGallery() {
        ...
        // Single shared observer untuk semua items — cegah memory leak
        ...
        photosToShow.forEach((photo, index) => {
            ...
            // DTREASURE: tambahkan token ke URL, jangan expose URL telanjang di DOM
            const lazySrc = isDtreasure
            ...
            // Tentukan label download button
            let btnIcon, btnText;
            if (isDtreasure) {
                const isFree = state.lifetime || state.purchasedImages.has(photo.id);
                btnIcon = isFree ? 'fa-download' : 'fa-lock';
                btnText = isFree ? 'Download'   : '10 Credits';
            } else {
                btnIcon = 'fa-download';
                btnText = 'Download';
            }

            // Proteksi drag & klik-kanan untuk DTREASURE
            const imgAttrs = isDtreasure
    ...
    // FIX S-19: Setelah spend credit berhasil, update SEMUA gallery yang menampilkan foto ini
    // (bukan hanya renderDtreasureGallery — foto DTREASURE bisa muncul di search results juga)
    async handleDownload(photo) {
        ...
        if (isDtreasure) {
            ...
            if (result.success) {
                ...
                // Refresh dedicated DTREASURE section
                ui.renderDtreasureGallery();

                // FIX S-19: Refresh renderGallery juga jika foto ini ada di filteredPhotos
                // (terjadi saat user sedang di search results)
                const hasDtreasureInFiltered = state.filteredPhotos.some(p => p.id === photo.id);
                if (hasDtreasureInFiltered) {
                    ui.renderGallery();
                }

                // Update modal download button jika modal masih terbuka untuk foto ini
                if (state.currentImageId === photo.id && elements.downloadBtn) {
                    elements.downloadBtn.innerHTML = '<i class="fas fa-download mr-2"></i>Download';
                }
            } else {
    ...
    // FIX S-6:
    // - Blob URL di-revoke setelah 1000ms (bukan 5000ms) — cegah re-access
    // - Non-DTREASURE: hapus target="_blank" agar URL tidak tampil di tab inspectable
    async triggerDownload(photo) {
        ...
        // Strip semua query params yang mungkin sudah ada di photo.url sebelum append baru
        const cleanUrl    = (photo.url.startsWith('http') ? photo.url : `${API_BASE}${photo.url}`).split('?')[0];
        ...
        if (isDtreasure) {
            ...
            try {
                ...
                // FIX S-6: Revoke setelah 1000ms — cegah user akses ulang URL blob
            ...
            // FIX S-6: Hapus target="_blank" — cegah URL tampil di tab yang bisa diinspeksi
            const a    = document.createElement('a');
            a.href     = downloadUrl;
            a.download = filename;
            document.body.appendChild(a);
    ...
    // FIX S-5: Single shared IntersectionObserver untuk renderComitbaseGallery
    renderComitbaseGallery() {
        ...
        // Single shared observer — cegah memory leak
        const imageObserver = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const img = entry.target;
    ...
    // FIX S-4:
    // - Single shared IntersectionObserver (bukan satu per item — memory leak)
    // - Token ditambahkan ke data-src DTREASURE preview
    // - img DTREASURE: draggable=false + oncontextmenu=return false
    renderDtreasureGallery() {
        ...
        // Single shared observer untuk semua items DTREASURE
        const imageObserver = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const img = entry.target;
        ...
        photosToShow.forEach((photo, index) => {
            ...
            // Tambahkan token ke preview URL — jangan expose URL telanjang di DOM
            const lazySrc = `${baseUrl}?token=${encodeURIComponent(token)}`;
    ...
    async loadQuicklinks() {
        try {
            ...
            const list = document.getElementById('quicklinks-list');
            if (list) list.innerHTML = '<p class="text-center text-gray-500 py-4">Failed to load links</p>';
        }
    },
...
function initEvents() {
    ...
    Object.entries(categoryMap).forEach(([id, category]) => {
        ...
        if (btn) btn.addEventListener('click', () => ui.changeCategory(category));
    });
    ...
    if (elements.searchInput)       elements.searchInput.addEventListener('input', debouncedSearch);
    if (elements.searchInputMobile) elements.searchInputMobile.addEventListener('input', debouncedSearch);
